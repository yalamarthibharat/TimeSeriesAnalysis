...
for i = 3:n
    zhat(i) = c + (phi1 * zhat(i-1)) + (theta1 * epsilon(i-1))+ (theta2 * epsilon(i-2)) + epsilon(i);
end
residual = z - zhat;
residual_mean = mean(residual);
residual_variance = var(residual);
figure(13);
plot(time_co2,residual);
title('Plot of residual');
figure(14);
histogram(residual);
title('Histogram plot of residual');
figure(15);
autocorr(residual,380);
figure(16);
parcorr(residual,380);
